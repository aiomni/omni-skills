...
description: 拆解和执行研发任务。用户提到需求拆解、实施计划、里程碑、依赖、关键路径、验收标准、任务卡、dashboard、执行日志、Review、inbox，或者要求把模糊工程目标落成可执行任务系统时，必须使用此 skill。也适用于为现有研发工作补任务卡、同步真实进展、判断是否拆卡或调整优先级，即使用户没有明确说“建 tasks 目录”。
---
...
这些模板来自当前仓库 `tasks/` 的成熟结构，但已经去掉项目特定内容，适合在其他 repo 中直接落地后再按实际项目改写。
...
如果目标仓库已经有自己的状态机，沿用原系统，但保留本 skill 的验收与同步原则。
