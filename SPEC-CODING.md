...
当用户已经安装 `writing-spec`、`writing-plan` 和 `writing-tasks`，并希望后续 AI agent 在目标项目中默认按 Spec Coding 工作时，使用 `omni-writing-agentsmd`。

它关注：
- 识别目标用户项目，而不是 `omni-skills` 仓库本身
- 创建或更新目标项目的 `AGENTS.md`
- 将 `Spec -> Plan -> Task`、current first、small context cut、向上回写和层级 hook 写入 agent 默认规则
- 保留项目已有的 `AGENTS.md` 指令和更窄作用域规则

安装：

```bash
npx skills add https://github.com/aiomni/omni-skills --skill omni-writing-agentsmd
```
...
安装 `writing-spec`、`writing-plan` 和 `writing-tasks` 后，用 `omni-writing-agentsmd` 把 Spec Coding 的运行规则写入目标用户项目的 `AGENTS.md`。

这一步只修改目标项目的 `AGENTS.md`，不是修改 `omni-skills` 仓库的 `AGENTS.md`。它的作用是让后续进入该项目的 AI agent 默认理解：当前真相读 Spec，实施策略读 Plan，此刻执行读 Task。
...
for skill in writing-spec writing-plan writing-tasks omni-writing-agentsmd; do
