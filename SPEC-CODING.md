...
当用户已经安装 `writing-spec`、`writing-plan` 和 `writing-tasks`，并希望后续 AI agent 在目标项目中默认按 Spec Coding 工作时，使用 `omni-writing-agents`。
...
```bash
npx skills add https://github.com/aiomni/omni-skills --skill omni-writing-agents
```
...
安装 `writing-spec`、`writing-plan` 和 `writing-tasks` 后，用 `omni-writing-agents` 把 Spec Coding 的运行规则写入目标用户项目的 `AGENTS.md`。
...
for skill in writing-spec writing-plan writing-tasks omni-writing-agents; do
